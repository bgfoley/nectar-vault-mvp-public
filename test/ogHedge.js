...
async function main() {
    ...
    const shares = (depositWei * slippage * price) / BigInt(1e15); // Adjusted to 1e18 precision
    ...
    if (execute.toLowerCase() === 'y') {
        try {
            const tx = await hedgeContract.hedge(depositWei, shares, price, { value: depositWei + EXECUTION_FEE });
