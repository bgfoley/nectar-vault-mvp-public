...
async function main() {
    ...
    // Query user for shares and slippage
    const shares = readlineSync.question('Enter the number of shares to unHedge: ');
    const slippageTolerance = readlineSync.question('Enter your slippage tolerance (e.g., 0.01 for 1%): ');

    const sharesWei = BigInt(shares); // Convert shares to BigInt
    const slippage = BigInt(Math.round(parseFloat(slippageTolerance) * 1000 + 1000));

    // Fetch the price of WETH
    const prices = await getSignedPrices(WETH);
    const acceptablePrice = prices.price;
    const precision = BigInt(1e18);

    // Calculate the value of outgoing shares in ETH
    const outgoingValue = sharesWei / acceptablePrice * precision;
    const amountWei = outgoingValue / slippage / BigInt(1000);
    ...
    if (execute.toLowerCase() === 'y') {
        try {
            ...
            const gasEstimate = await hedgeContract.unHedge.estimateGas(sharesWei, amountWei, acceptablePrice, { value: EXECUTION_FEE });
            console.log(`Estimated gas: ${gasEstimate.toString()}`);

            // Call the unHedge function
            const tx = await hedgeContract.unHedge(sharesWei, amountWei, acceptablePrice, { value: EXECUTION_FEE, gasLimit: gasEstimate });
