 ...
 */
contract ShMdge is ERC20, Ownable, IOrderCallbackReceiver {
...
/*
    function hedge(
        ...
        uint256 executionFee = 3000000000000; // Assumed execution fee in wei (3e-06 ETH) - includes callback gas limit
        uint256 callBackGasLimit = 1500000000000;
        uint256 initialCollateralDeltaAmount = amount - executionFee - callBackGasLimit;
        uint256 sizeDeltaUsd = (initialCollateralDeltaAmount * acceptablePrice) / 1e24; // Adjusted for 24 decimals
        ...
        IBaseOrderUtils.CreateOrderParamsNumbers memory orderParamsNumbers = IBaseOrderUtils.CreateOrderParamsNumbers({
            ...
            callbackGasLimit: 1500000000000, 
            minOutputAmount: 0
        });
...
*/
    ...
    function hedge(
    ...
    ) external payable returns (bytes32 key, address orderAccount) {
        require(msg.value >= orderParams.numbers.executionFee, "Insufficient execution fee");
        ...
        key = Router(GMX_ROUTER).createOrder{value: msg.value}(orderParams);
   ...
   /**
    ...
    function unHedge(
        ...
        uint256 initialCollateralDeltaAmount = (sharesToUsd(shares) * 1e18) / acceptablePrice; // Convert shares to collateral amount
        ...
        IBaseOrderUtils.CreateOrderParamsNumbers memory orderParamsNumbers = IBaseOrderUtils.CreateOrderParamsNumbers({
            ...
            acceptablePrice: acceptablePrice,
            executionFee: executionFee,
            callbackGasLimit: 3000000000000, // Set to 3 million gas
            minOutputAmount: 0
        });
     ...
     *      NOTE: visibility set to public for testing purposes. Must be internal for production
    ...
    function _unHedge(
    ...
    ) public returns (bytes32 key, address orderAccount) {
        ...
        // Execution fee plus callbackGasLimit
        uint256 totalFee = orderParams.numbers.executionFee + orderParams.numbers.callbackGasLimit;

        // Transfer execution fee to orderVault
        IERC20(WETH).safeTransferFrom(user, ORDER_VAULT, totalFee);
