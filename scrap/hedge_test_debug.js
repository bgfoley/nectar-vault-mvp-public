...
async function main() {
  try {
    ...
    const executionFeeBn = ethers.parseUnits(await getExecutionFee(), 'ether');

    // Step 4: Calculate sizeDeltaUSD and initialCollateralDeltaAmount
    const initialCollateralDeltaAmount = depositEthBn.sub(executionFeeBn);
    ...
    console.log(`Execution Fee: ${ethers.formatUnits(executionFeeBn, 18)} ETH`);
    ...
    console.log(`Initial Collateral Delta Amount: ${initialCollateralDeltaAmount.toString()}`);
