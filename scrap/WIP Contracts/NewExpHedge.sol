...
/**    
 *   /// @dev Accessing specific variables from eventData
 *   uint256 collateralDeltaAmount = eventData.uintItems.items[4].value; // collateralDeltaAmount
 *   uint256 sizeDeltaInTokens = eventData.uintItems.items[12].value; // sizeDeltaInTokens
 *   uint256 sizeDeltaUsd = eventData.uintItems.items[2].value; // sizeDeltaUsd
 *   uint256 executionPrice = eventData.uintItems.items[17].value; // executionPrice
 *   uint256 collateralAmount = eventData.uintItems.items[10].value; // collateralAmount
 *   uint256 sizeInTokens = eventData.uintItems.items[6].value; // sizeInTokens
 *   uint256 sizeInUsd = eventData.uintItems.items[1].value; // sizeInUsd
 *
 *   // Example: Store collateralDeltaAmount for further use
 *   wethBalance[_user] += collateralDeltaAmount;
 *
 */
    
contract NExpHdg is ERC20, Ownable, IOrderCallbackReceiver {
    ...
    /// @dev to log state changes after unHedge order execution
    ...
    /// @dev For emergency purposes only
    bool public paused;

    /// Contract State Variables

    /// @dev Hedge contract's collateral balance on exchange
    uint256 public collateralBalance;

    /// @dev Hedge contract's position size on exchange
    uint256 public positionSizeUsd;

    /// @dev Hedge contract's position size on exchange
    uint256 public positionSizeWeth;
    ...
    /// @dev exchange related addresses
    ...
    /// @dev Fee is added to msg.value to place orders on exchange
    uint256 private constant EXECUTION_FEE = 114000000000000; 

    /// @dev Fee factor handles accounting
    /// for exchange fees: ~ .05% on market increase orders, 
    ///     ~ .07% on market decrease orders. Hedge rounds up to .1% 
    uint256 private FEE_FACTOR = 999000000000000000;

    /// @dev GMX rolestore to verify origin of afterOrderExecution calls
...
*/
     ...
     */
    constructor() ERC20("NExpHdg", "HDG") Ownable(msg.sender) {}
     ...
     * @notice Initializes the contract with necessary addresses
     * @dev This function must be called after deployment to set the required addresses
     * @param _roleStore The address of the role store contract
     * @param _receiver The address to receive the orders
     * @param _callbackContract The address of the callback contract
     * @param _uiFeeReceiver The address to receive UI fees
     * @param _market The address of the GMX market
     * @param _initialCollateralToken The address of the initial collateral token
     * @param _swapPath The array of addresses for the swap path
     */
    ...
    function _hedge(
     ...
     function unHedge(
        ...
        /// @dev this is just here for debugging - remove for production
    ...
    function _unHedge(
        ...
        IERC20(HEDGE_VAULT).safeTransferFrom(user, address(this), shares);
    ...
    function afterOrderExecution(bytes32 key, Order.Props memory order, EventUtils.EventLogData memory eventData) external override {
      ...
      //  require(roleStore.hasRole(msg.sender, GMX_CONTROLLER), "Invalid role");
        ...
        uint256 _initialCollateralDeltaAmount = order.numbers.initialCollateralDeltaAmount;
        ...
        if (order.numbers.orderType == Order.OrderType.MarketIncrease) {
            ...
            emit OrderExecuted(_user, key, _initialCollateralDeltaAmount);
            ...
            uint256 _wethReturned = _initialCollateralDeltaAmount * FEE_FACTOR / 10e18;
            wethBalance[_user] += _wethReturned;


            // Hold WETH in contract 
            // Log this event for later reconciliation
            emit OrderExecuted(address(this), key, _initialCollateralDeltaAmount);
            ...
            collateralBalance = eventData.uintItems.items[10].value;
            positionSizeUsd = eventData.uintItems.items[1].value;
            positionSizeWeth = eventData.uintItems.items[6].value;
    ...
    //    _removeOrder(key, _user);
        ...
        if (order.numbers.orderType == Order.OrderType.MarketIncrease) {
            ...
            IERC20(WETH).safeTransfer(_user, _initialCollateralDeltaAmount);
        } else if (order.numbers.orderType == Order.OrderType.MarketDecrease) {
            uint256 shares = usdToShares(order.numbers.sizeDeltaUsd);
            
            // Unlock share
            lockedShares[_user] -= usdToShares(shares);
            IERC20(HEDGE_VAULT).safeTransfer(_user, shares);
        }

        // Emit order cancelled event
        emit OrderCancelled(_user, key, _initialCollateralDeltaAmount);
    }
