...
async function main() {
  ...
  try {
    const hedgeTx = await hedge.ownerPlaceOrder(orderParams, depositEthBn, { value: executionFeeBn });
    await hedgeTx.wait();
    console.log("ownerPlaceOrder function executed successfully");
  } catch (error) {
    console.error("Error executing ownerPlaceOrder function:", error);
