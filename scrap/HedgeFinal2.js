...
async function main() {
  ...
  try {
    ...
    const baseGasLimit = BigInt(await dataStore.getUint(execution_gas_fee_base_amount_key));
    const multiplierFactor = BigInt(await dataStore.getUint(execution_gas_fee_multiplier_key));
    const estimatedGasLimit = BigInt(await dataStore.getUint(increase_order_gas_limit_key));
    ...
    const executionFeeEth = executionFeeWei / 10n**18n; // Convert to ETH
    console.log(`Execution fee estimated: ${executionFeeEth.toString()} ETH`);
