 ...
 */
contract PaySmdge2 is ERC20, Ownable, IOrderCallbackReceiver {
...
*/
    ...
    function calculatePriceFromEth(uint256 amount, uint256 minimumSharesOut) internal pure returns (uint256) {
        // Using a larger precision to avoid fractional errors
     //   uint256 precision = 1e18; 
        uint256 slippage = 1003; // 1003 represents 1.003 for a 0.3% slippage

        // Calculate price in 1e18 precision
 ...
 // Default slippage .003
    function hedge(
        ...
        uint256 minimumSharesOut, // Shares out (Usd)
    ...
    function unHedge(
        uint256 amount, // shares in
        uint256 minimumOut, // Eth out
        uint256 acceptablePrice
    ) external payable whenNotPaused returns (bytes32 key, address orderAccount) {      
        // Validate order params
      //  _validateOrderParams(amount, minimumOut);
...
/*
    ...
    function hedge(
     ...
     //   _validateOrderParams(amount, minimumSharesOut);
        ...
        uint256 acceptablePrice = calculatePriceFromEth(amount, minimumSharesOut);

        // Calculate sizeDeltaUsd with 1e30 precision for GMX
        uint256 sizeDeltaUsd = (initialCollateralDeltaAmount * acceptablePrice); // Adjusted for 30 decimals
...
/*
    function unHedge(
      ...
      //  _validateOrderParams(amount, minimumOut);
        ...
        uint256 acceptablePrice = calculatePriceFromShares(amount, minimumOut);
