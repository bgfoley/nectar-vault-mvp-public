...
// File contracts/HedgeCurrent.sol
 ...
 */
contract Booge is ERC20, Ownable, IOrderCallbackReceiver {
...
*/    
 ...
 /* INSERT Nectar DataReader Interface
...
*/
     ...
     */
    constructor() ERC20("Booge", "BULL") Ownable(msg.sender) {}
...
*/
    ...
    function afterOrderExecution(bytes32 key, Order.Props memory order, EventUtils.EventLogData memory eventData) external override {
     ...
     //  _removeOrder(key, _user);
        ...
        // For market increase order (hedge), mint shares
    ...
    //    _removeOrder(key, _user);
        ...
        // If decrease order then return the unspent shares to user
        } else if (order.numbers.orderType == Order.OrderType.MarketDecrease) {
            
            uint256 _lockedShares = lockedShares[_user];
            
            uint256 unspentShares = sharesToUsd(order.numbers.sizeDeltaUsd);

            if (_lockedShares >= unspentShares) {
            
            // Unlock shares
            _lockedShares -= unspentShares;

            // Return shares to user
            IERC20(HEDGE_VAULT).safeTransfer(_user, unspentShares);
            
            } 
            // need to consider handling of lack of lockedShares, if it's possible
          //  else {
          //      return "No shares to return";
            }
     ...
     * @param price The acceptable price for the order
     */
    function _validateOrderParams(uint256 amount, uint256 price) internal pure {
        // Ensure the amount is greater than zero
        require(amount > 0, "Invalid deposit amount");
        // Ensure the acceptable price is greater than zero
        require(price > 0, "Invalid acceptable price");
        ...
        // require(price >= MIN_ACCEPTABLE_PRICE && price <= MAX_ACCEPTABLE_PRICE, "Acceptable price out of range");
    }
