...
contract ShortFundingBot {
    ...
    function storeAndDeposit(
        ...
        return deposit(amount0, orderParams, swapMinAmount);
    }

    function deposit(uint256 amount0, CreateOrderParams calldata orderParams, uint256 swapMinAmount) public returns (uint256) {
        if (msg.sender == OWNER) {
    ...
    function withdrawAndEndBot(uint256 amount0, CreateOrderParams calldata orderParams, uint256 swapMinAmount) external returns (uint256) {
        ...
        uint256 amount = _withdraw(amount0, orderParams
